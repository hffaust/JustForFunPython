...
import utils
# THis is the maximum number of instructions we will log after an access violation
MAX_INSTRUCTIONS = 10

# This is far from and exhaustive list... just a start
dangerous_functions = {
                       "strcpy"   : "msvcrt.dll",
                       "strncpy"  : "msvcrt.dll",
                       "sprintf"  : "msvcrt.dll",
...
def danger_handler(dbg):
    ...
    # Generally there are only going to be a few parameters, 
    # so we will take everything from ESP to ESP+20,
    # which should give us enough information to determine 
    # if we own any of the data
    esp_offset = 0
    print "[*] Hit %s" % dangerous_functions_resolved[dbg.context.Eip]
    print "==============================================================="
    
    while esp_offset <= 20:
        paramater = dbg.smart_dereference(dbg.context.Esp + esp_offset)
        print "[ESP + %d] => %s" % (esp_offset, paramater)
        esp_offset += 4
    print "==============================================================="
...
def access_violation_handler(dbg):
    ...
    # Something bad happened, which means something good happened for us :)
